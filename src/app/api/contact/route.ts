...
export  async function POST(req:NextRequest) {
  ...
  try {
    ...
    // Email content
    const mailOptions = {
      from: `📧 SmartBriefs ${process.env.OTP_EMAIL_USER}`,
      to: process.env.EMAIL_USER, // Your email address where you want to receive contacts
      subject: `📧 New Message from ${name} via SmartBriefs!`,
      html: `
      <div style="font-family: 'Segoe UI', sans-serif; background: #f9f9ff; padding: 24px; border-radius: 12px;">
        ...
        <p style="font-size: 1.1em;">Hi there! <strong>${name}</strong> just sent you a message through the SmartBriefs contact form. 🎉</p>
        ...
        <p style="margin-top: 24px; color: #888;">Sent with 🫀 from your SmartBriefs website!</p>
      </div>
      `,
    };
